...
public class EntityInfoPorting
{
	...
	public static List<BaseText> entityInfo(Entity e, World source_world)
	{
		...
		if (e.method_20802() > 0) { lst.add(Messenger.c("w  - Fire for ","wb "+e.method_20802(),"w  ticks")); }
		...
		if (e instanceof ItemEntity)
		{
			...
			lst.add(Messenger.c("w  - Despawn Timer: ", "wb " + EntityInfoAccessor.makeTime(((ItemEntityAccessor)ei).getAge())));
		}
		if (e instanceof ExperienceOrbEntity)
		{
			ExperienceOrbEntity exp = (ExperienceOrbEntity)e;
			lst.add(Messenger.c("w  - Despawn Timer: ", "wb "+EntityInfoAccessor.makeTime(exp.orbAge)));
...
//							}
						...
						if (e instanceof HorseBaseEntity)
						{
							...
							lst.add(Messenger.c(String.format("w  - Horse Speed: %.2f b/s (%.1f%%%%)",
									EntityInfoAccessor.get_speed(elb.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).getValue()),
									EntityInfoAccessor.get_horse_speed_percent(elb.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).getValue())
							)));
