...
查询或修改给定区块的玩家停留总刻数（inhabited time）。

`<刻数>` 需为非负整数。`<增量刻数>` 可为正数或负数。`query` 会输出所选区块的最小值、最大值和平均值，`add` 则会在应用增量后输出更新后的统计值。

| 指令                                                | 效果                |
|---------------------------------------------------|-------------------|
| `/manipulate chunk inhabitedTime query <目标>`      | 查询目标区块的玩家停留总刻数    |
| `/manipulate chunk inhabitedTime set <刻数> <目标>`   | 设置目标区块的玩家停留总刻数    |
| `/manipulate chunk inhabitedTime add <增量刻数> <目标>` | 增加或减少目标区块的玩家停留总刻数 |
...
|--------------------------|---------------------------------|
| `current`                | 你所在的区块                          |
| `square\|chebyshev <半径>` | 给定半径内的所有区块（切比雪夫距离），半径范围为 0 到 32 |
| `circle\|euclidean <半径>` | 给定半径内的所有区块（欧几里得距离），半径范围为 0 到 32 |
| `at <区块X> <区块Z>`         | 指定位置的区块。给定区块需已被加载               |
