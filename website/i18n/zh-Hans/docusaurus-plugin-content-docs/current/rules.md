...
移植 Minecraft 1.21.5+ 版本中的特性：区块刻可在任何强加载区块中执行，无需区块附近存在玩家
...
不过你仍可以通过该选项启用“perTick”动作模式，或限制其仅允许管理员玩家使用
...
可与选项 [dispensersFireDragonBreath](#发射器发射龙息-dispensersfiredragonbreath) 联动
...
阻止服务端因更新抑制造成的 `StackOverflowError`、`OutOfMemoryError`、`ClassCastException` 或 `IllegalArgumentSuppression` 异常而导致的服务器崩溃
